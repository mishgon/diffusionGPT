...
from diffusion_gpt_model import DiffusionGPTConfig, DiffusionGPT

# -----------------------------------------------------------------------------
# default config values designed to train a gpt2 (124M) on OpenWebText
# I/O
out_dir = 'out/diffusion-gpt-124M-owt'
eval_interval = 2000
log_interval = 1
eval_iters = 200
...
wandb_project = 'owt'
wandb_run_name = 'diffusion-gpt-124M' # 'run' + str(time.time())
# tensorboard logging
tb_log = True
# data
dataset = 'openwebtext'
...
if meta_vocab_size is not None:
    vocab_size = meta_vocab_size + 2 # add MASK and CLS tokens
else:
...
def get_batch(split):
    ...
    input_ids = torch.stack([torch.from_numpy((data[i:i+block_size]).astype(np.int64)) for i in ix])
    if device_type == 'cuda':
    ...
    p_mask = (1 - eps) * t + eps
    mask = torch.rand((batch_size, block_size), device=device) < p_mask.unsqueeze(1)
    target_ids = input_ids
    mask_token_id = vocab_size - 2
    input_ids = torch.where(mask, mask_token_id, input_ids)
    return input_ids, target_ids, mask, p_mask
...
if init_from == 'scratch':
    ...
    if model_args['n_lm_heads'] > 1 and model_args['cce_impl'] == 'none':
        warnings.warn('When using `--n_lm_heads=*`, consider also using `--cce_impl=*` to reduce GPU memory footprint')
...
assert block_size == model.config.block_size, 'model.crop_block_size is needed'
model.to(device)
